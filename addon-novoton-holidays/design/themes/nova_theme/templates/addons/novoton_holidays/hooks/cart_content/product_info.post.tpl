...
<div style="margin-top:10px;padding:12px;background:#f5f9fc;border-radius:6px;font-size:13px;border-left:4px solid #003580;">
    ...
    <div style="margin-bottom:6px;"><strong></strong> {$product.extra.check_in|default:''|date_format:"%d.%m.%Y"} - {$product.extra.check_out|default:''|date_format:"%d.%m.%Y"} ({$product.extra.nights} {__("novoton_holidays.nights")|default:"nopți"})</div>
    {if $product.extra.num_rooms > 1 && $product.extra.rooms_data}
        ...
        {foreach from=$product.extra.rooms_data item=room name=nv_rooms}
            <div style="margin-left:10px;margin-bottom:6px;padding:6px 8px;background:#fff;border-radius:4px;border:1px solid #e0e7ef;display:flex;justify-content:space-between;align-items:flex-start;">
                <div style="flex:1;">
                    <strong>Room {$smarty.foreach.nv_rooms.iteration}:</strong> {$room.room_type_display|default:$room.room_name|default:$room.room_id|escape:html}
