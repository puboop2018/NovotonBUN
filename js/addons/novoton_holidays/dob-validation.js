...
(function() {
    ...
    // =========================================================================
    // SHARED DATE PARSING — delegates to NovotonUtils when available
    // =========================================================================

    function parseDate(value) {
        if (window.NovotonUtils && window.NovotonUtils.parseDate) {
            return window.NovotonUtils.parseDate(value);
        }
        // Inline fallback (same logic as NovotonUtils.parseDate)
    ...
    function calculateAge(dob, targetDate) {
        if (window.NovotonUtils && window.NovotonUtils.calculateAge) {
            return window.NovotonUtils.calculateAge(dob, targetDate);
        }
        ...
        if (roomMatch && childMatch) {
            ...
            if (ageDisplay) {
                ...
                if (age >= 18) {
                    ...
                    ageDisplay.appendChild(document.createTextNode(' ' + ((window.NovotonTranslations && window.NovotonTranslations.childMustBeUnder18) || 'Must be under 18')));
                } else {
    ...
    function showPriceChangeNotification(text, type) {
        ...
        var label = (type === 'increase')
            ? ((window.NovotonTranslations && window.NovotonTranslations.priceIncreased) || 'Price increased')
            : ((window.NovotonTranslations && window.NovotonTranslations.priceDecreased) || 'Price decreased');
    ...
    window.Novoton.initDOBValidation = initDOBValidation;
    window.Novoton.fixVisibility = fixDesktopMobileVisibility;
    window.Novoton.validateAllDOBFields = validateAllDOBFields;
    window.Novoton.validateDOBForCheckIn = validateDOBForCheckIn;
