...
/**
 * Mode: test_resorts
 * Test resort list sync
 */
if ($mode == 'test_resorts') {
    if (!fn_check_permissions('manage_catalog', 'update', 'admin')) {
        return [CONTROLLER_STATUS_DENIED];
    }
    ...
    $country = $_REQUEST['country'] ?? 'BULGARIA';
    echo '<h2>Resort List Sync Test (' . htmlspecialchars($country) . ')</h2>';
    ...
    if ($result['success']) {
        ...
        echo '<p>Removed: ' . $result['removed'] . '</p>';
    } else {
        echo '<p style="color:red">Failed: ' . ($result['error'] ?? 'Unknown error') . '</p>';
    }

    // Show current resorts vs DB cities
    $resorts = db_get_array("SELECT * FROM ?:novoton_resorts WHERE country = ?s ORDER BY resort_name", $country);
    ...
    $resort_names = array_column($resorts, 'resort_name');

    // Find mismatches
    $in_api_not_db = array_diff($resort_names, $db_cities);
    $in_db_not_api = array_diff($db_cities, $resort_names);

    if (!empty($in_api_not_db) || !empty($in_db_not_api)) {
        echo '<h3 style="color:orange">City/Resort Mismatches:</h3>';
        if (!empty($in_api_not_db)) {
            echo '<p><strong>In API resort_list but NOT in DB cities:</strong></p><ul>';
            foreach ($in_api_not_db as $r) {
                echo '<li>' . htmlspecialchars($r) . '</li>';
            }
            echo '</ul>';
        }
        if (!empty($in_db_not_api)) {
            echo '<p><strong>In DB cities but NOT in API resort_list:</strong></p><ul>';
            foreach ($in_db_not_api as $c) {
                $count = db_get_field("SELECT COUNT(*) FROM ?:novoton_hotels WHERE city = ?s AND country = ?s", $c, $country);
                echo '<li>' . htmlspecialchars($c) . ' (' . $count . ' hotels)</li>';
            }
            echo '</ul>';
        }
    }

    echo '<h3>API Resorts (' . count($resorts) . '):</h3>';
    echo '<table border="1" cellpadding="5">';
    echo '<tr><th>Resort Name</th><th>Country</th><th>Synced At</th></tr>';
    foreach ($resorts as $r) {
        $in_db = in_array($r['resort_name'], $db_cities);
        $style = $in_db ? '' : ' style="background:#fff3cd"';
        echo '<tr' . $style . '>';
        echo '<td>' . htmlspecialchars($r['resort_name']) . ($in_db ? '' : ' *') . '</td>';
        echo '<td>' . htmlspecialchars($r['country']) . '</td>';
        echo '<td>' . htmlspecialchars($r['synced_at'] ?? '') . '</td>';
        echo '</tr>';
    }
    echo '</table>';
    echo '<p><small>* = resort exists in API but no hotels in DB have this city</small></p>';
