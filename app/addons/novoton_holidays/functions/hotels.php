...
/**
 * Sync resorts from API
 * 
 * @param array $countries Countries to sync
 * @return array Result with counts
 */
function fn_novoton_sync_resorts_from_api($countries = [])
{
    $api = fn_novoton_get_api();
    if (!$api) {
        return ['success' => false, 'error' => 'API not available'];
        ...
        try {
            ...
            if (!empty($hotels)) {
                foreach ($hotels as $hotel) {
                    ...
                    if (!empty($hotel_id) && !empty($resort)) {
                        db_query(
                            "UPDATE ?:novoton_hotels SET resort = ?s WHERE hotel_id = ?s",
                            $resort, $hotel_id
                        );
                        $result['synced']++;
                    }
                ...
                $result['countries'][$country] = count($hotels);
            }
 ...
 * @return array Stats
 */
function fn_novoton_update_resort_stats()
{
    $stats = db_get_array(
        "SELECT country, resort, COUNT(*) as hotel_count
         FROM ?:novoton_hotels
         WHERE resort IS NOT NULL AND resort != ''
         GROUP BY country, resort
         ORDER BY country, hotel_count DESC"
    );
