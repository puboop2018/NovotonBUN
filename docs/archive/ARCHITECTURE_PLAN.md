...
2. **Other core-consolidation targets** (higher value than the repo interface):
   ...
   | `Container` / `SyncLogRepository` | DI + sync logging re-implemented per addon | **🔬 Re-evaluated 2026-07-04: deferred.** The two repos share zero method signatures and the two sync-log *tables* differ materially (`sync_date`/`products_*` vs `started_at`/`rate_limit_hits`/`sync_mode`) — a shared base needs schema unification first (schedule with the migration-runner work). `Container` stays per-provider by design (DI wiring is provider-specific). |
   | `PreOrderPriceVerifier` price policy | "No Surprises" checkout rule duplicated (and drifted) per provider | **✅ Done 2026-07-04:** `TravelCore\Services\PriceComparisonPolicy` (+ DTO/enums, 15 tests); both verifiers delegate case selection with their historical knobs explicit at the call site. Full verifier-skeleton unification rejected — flows genuinely diverge (sphinx unavailable-bucket, novoton session cache + detector alerts). |
   | `CronDispatcher` | Parallel dispatchers per addon | **🔬 Re-evaluated 2026-07-04: rejected as an extraction.** Different concurrency models (sphinx: file-lock machinery; novoton: none, glob auto-discovery, different command DI). Follow-up is a *feature*: port sphinx's file-lock protection to novoton's cron. |
