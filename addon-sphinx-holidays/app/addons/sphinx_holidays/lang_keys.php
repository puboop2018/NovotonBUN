...
return [
    ...
    'sphinx_holidays.require_immediate_availability.tooltip' => [
        'en' => 'When enabled, the admin hotel list shows only hotels with at least one confirmation=immediate offer — the hotels cron flags the rest during sync, hides them from the list and skips them at product creation until they become available — and the storefront search shows only immediate-confirmation offers. The hotels cron accepts a per-run override: &availability_gate=0 to skip the gate, &availability_gate=1 to force it.',
        'ro' => 'Când este activ, lista de hoteluri din admin afișează doar hotelurile cu cel puțin o ofertă confirmation=immediate — cronul de hoteluri le marchează pe celelalte la sincronizare, le ascunde din listă și le omite la crearea produselor până devin disponibile — iar căutarea din storefront afișează doar ofertele cu confirmare imediată. Cronul de hoteluri acceptă o suprascriere per rulare: &availability_gate=0 pentru a omite filtrul, &availability_gate=1 pentru a-l forța.',
    ],
